...
class TestHashLookup:
    ...
    # verifies that the digest is domain separated, so a short value cannot be
    # recovered from a table of bare sha256 digests
    def test_is_not_a_bare_sha256(self) -> None:
        bare = hashlib.sha256(b"00000001r").hexdigest()
        prefixed = hashlib.sha256(b"qrew-pii-v1:00000001r").hexdigest()
        assert hash_lookup("00000001r") != bare
        assert hash_lookup("00000001r") == prefixed
