...
from com.qode.qrew.v1.entry.repositories.entry_attempt import EntryAttemptRepository
...
from com.qode.qrew.v1.entry.services.domain.entry import EntryOutcome, EntryReason
...
async def validate_entry(
    ...
    *,
    ...
    if scanner_event_id is not None:
        try:
            await EntryAttemptRepository(session).record(
                EntryAttempt(
                    id=uuid.uuid4(),
            ...
            await logger.awarning("scan_record_failed", error=repr(exc))
    return decision
...
async def _evaluate(
    *,
    ...
    try:
        from contracts.messaging.envelope import (
