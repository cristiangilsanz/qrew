...
from com.qode.qrew.v1.identity.models.audit.audit import AuditAction, AuditEvent
...
def compute_hash(prev_hash: bytes | None, event_data: dict[str, object]) -> bytes:
    """Compute the chained hash for an audit event."""
...
def event_to_hashable(event: AuditEvent) -> dict[str, object]:
    """Return the audit event fields that contribute to the hash."""
    return {
        "id": str(event.id),
