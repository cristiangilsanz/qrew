...
from com.qode.qrew.v1.identity.worker.jobs.registry import JobSpec
...
def _take_propagation_context(
    ...
    """Extracts and removes the trace propagation carrier from the incoming job arguments."""
    if not args:
...
def wrap_handler(spec: JobSpec) -> JobRunner:
    """Decorates a job handler with structured logging, distributed tracing, retry logic, and dead-letter routing."""

    async def runner(ctx: dict[str, Any], *args: Any, **kwargs: Any) -> Any:
        attempt = int(ctx.get("job_try", 1))
        job_id = str(ctx.get("job_id", "unknown"))
        structlog.contextvars.clear_contextvars()
        structlog.contextvars.bind_contextvars(job_name=spec.name, job_id=job_id, attempt=attempt)
        args, carrier = _take_propagation_context(args, kwargs)
        parent_context = extract_context(carrier)
        try:
            with tracer.start_as_current_span(f"job.{spec.name}", context=parent_context) as span:
