...
from com.qode.qrew.v1.identity.core.locking.lua import RELEASE_SCRIPT
...
async def close_locking() -> None:
    """Release the shared Redis client used for distributed locks."""
...
class RedisLock:
    """One acquire/release cycle of a Redis-backed distributed mutex."""
    ...
    async def acquire(
        ...
        *,
        ...
        """Try to acquire the lock; jittered retries up to the budget."""
    ...
    async def release(self) -> None:
        """Delete the lock key if and only if our nonce still owns it."""
...
async def redlock(
    ...
    *,
    ...
    lock = RedisLock(
        ...
        else _settings.locking_default_ttl_seconds,
        redis_client=redis_client,
    )
    acquired = await lock.acquire(
        retry_attempts=retry_attempts
        if retry_attempts is not None
        else _settings.locking_default_retry_attempts,
        retry_delay_ms=retry_delay_ms
        if retry_delay_ms is not None
        else _settings.locking_default_retry_delay_ms,
    )
    if not acquired:
