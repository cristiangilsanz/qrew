...
def _score_for(now_ms: int, tiebreak: int) -> int:
    """Pack sale-instant first, then uniform random, then a deterministic tiebreak."""
...
async def join_queue(
    *,
    ...
    """Adds a user to the queue if not already present and returns their position."""
...
async def queue_position(event_id: uuid.UUID, user_id: uuid.UUID) -> int | None:
    """Return the caller's 1-based position, or None if not in the queue."""
    redis = _shared_client()
...
def _build_redeem_token(*, event_id: uuid.UUID, user_id: str) -> tuple[str, str]:
    ...
    return jwt_keys.sign(jwt_keys.QUEUE, payload), jti
...
async def redeem_window_token(*, token: str, user_id: uuid.UUID) -> str:
    """Validates a single-use redeem token and returns a reservation token."""
    from jwt import InvalidTokenError

    payload = jwt_keys.verify(jwt_keys.QUEUE, token)
    ...
    reservation_token, _reservation_jti = _build_reservation_token(
...
async def consume_reservation_token(*, token: str, user_id: uuid.UUID) -> uuid.UUID:
    """Validates and consumes a reservation token once and returns the associated event identifier."""
    from jwt import InvalidTokenError
