...
export function attachRefreshInterceptor(client: AxiosInstance): void {
  client.interceptors.response.use(
    ...
    async (error: unknown) => {
      ...
      // a call that carries a token of its own, such as the recovery ceremony, means
      // a different credential entirely, so replacing it would retry as the wrong caller
      if ((config as { _ownAuth?: boolean })._ownAuth) return Promise.reject(error)
