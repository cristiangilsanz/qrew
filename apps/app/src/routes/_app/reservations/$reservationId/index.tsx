...
import { Ban, CheckCircle2, ChevronDown, Clock, CreditCard, Loader2, Save } from 'lucide-react'
...
import { useReservationPayment } from '@/features/tickets/hooks/useReservationPayment'
...
function ReservationPage() {
  ...
  const { data: paymentStatus } = useReservationPayment(reservationId, !!clientSecret)
  ...
  // asks every cache that mirrors the purchase to read the server again
  useEffect(() => {
    if (!confirming) return
    void queryClient.invalidateQueries({ queryKey: ['reservation', reservationId] })
    void queryClient.invalidateQueries({ queryKey: ['reservation-payment', reservationId] })
    void queryClient.invalidateQueries({ queryKey: ['tickets'] })
  }, [confirming, queryClient, reservationId])
  ...
  useEffect(() => {
    ...
    // the events that issue the tickets cross two outbox drainers, so the wait is minutes long
    const timer = setTimeout(finish, 150_000)
  ...
  // the money has left the card but the tickets are still travelling through the event pipeline
  const paymentTaken = paymentStatus?.status === 'succeeded'
  ...
  return (
    <div className="mx-auto max-w-[430px] px-4 pt-5 pb-28">
      ...
      {confirming && (
        <div className="mt-6 flex flex-col items-center gap-3 py-10 text-center">
          ...
          <p className="text-muted-foreground max-w-[280px] text-xs">
            {t('tickets.payment.issuing')}
          </p>
        </div>
      )}

      {!isPaid && !confirming && paymentTaken && (
        <p className="mt-6 text-center text-sm font-medium text-yellow-400">
          {t('tickets.payment.issuing')}
        </p>
      )}
      {paymentFailed && (
        <p className="text-destructive mt-6 text-center text-sm">{t('tickets.payment.failed')}</p>
      )}
      {isExpired && (
      ...
      {!isPaid && !isCancelled && !isExpired && !countdownExpired && !clientSecret && (
