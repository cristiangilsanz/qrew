...
import { Calendar, CheckCircle2, ChevronRight, Clock, CreditCard, TicketX, X } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col pb-24">
        ...
        <div className="keyboard-hide fixed inset-x-0 bottom-24 z-40">
          <div className="mx-auto w-full max-w-[430px] space-y-3 bg-gradient-to-t from-[hsl(0,0%,10%)] to-transparent px-4 pt-8">
            <div className="flex items-center justify-between gap-3">
              ...
              <Link
                to="/market/offers/$offerId/checkout"
                params={{ offerId }}
                disabled={countdownExpired}
                className="bg-primary hover:bg-primary/90 flex h-14 shrink-0 items-center gap-2 rounded-full px-5 text-sm font-semibold text-white shadow-lg transition aria-disabled:opacity-40"
              >
      ...
      <ConfirmDialog
        ...
        tone="destructive"
        icon={X}
        title={t('market.offer.declineTitle')}
        description={t('market.offer.declineDesc')}
        irreversible
        confirmLabel={t('market.offer.decline')}
        isLoading={declineOffer.isPending}
        countdownSeconds={5}
        onConfirm={() => declineOffer.mutate()}
      />
    </div>
  )
}
