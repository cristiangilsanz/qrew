...
describe('displayTicketState', () => {
  ...
  // verifies that a paid reservation reads as processing until the ticket is issued
  it('reports a reserved ticket as processing once the reservation is paid', () => {
    expect(displayTicketState(ticket('reserved'), reservation('paid', FUTURE))).toBe('processing')
    expect(displayTicketState(ticket('reserved'), reservation('paid', PAST))).toBe('processing')
  })
