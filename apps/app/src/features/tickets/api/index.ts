...
// declares which gate checks the server will evaluate, so the client asks the
// holder for no more than the server is going to look at
export interface GateRequirements {
  reassertion: boolean
  geofence: boolean
  device_binding: boolean
  attestation: boolean
  location_integrity: boolean
}
...
export const ticketsApi = {
  ...
  // reads the payment status of a reservation, reconciled against the provider
  getReservationPayment: (reservationId: string) =>
    paymentsClient
      .get<PaymentStatusDetail>(`/v1/reservations/${reservationId}/payment`)
      .then((r) => r.data),

  // implements list tickets
  gateRequirements: () =>
    ticketingClient.get<GateRequirements>('/v1/tickets/qr/requirements').then((r) => r.data),
