...
# a sorted set keeps its score in an ieee-754 double, so a score above 2**53 is
# rounded when redis stores it. counting the milliseconds from a fixed epoch and
# leaving 14 low bits for the tiebreak keeps the whole score inside that mantissa
# until 2041, so the random tail survives the write instead of being
# rounded away and leaving the order to the member's own identifier
_QUEUE_EPOCH_MS = 1_704_067_200_000  # 2024-01-01T00:00:00Z
_TIEBREAK_BITS = 14
_TIEBREAK_MASK = (1 << _TIEBREAK_BITS) - 1
MAX_EXACT_SCORE = 1 << 53


# derives a sortable score that orders arrival time before a random tiebreak,
# mixing fresh entropy into the caller's tiebreak so that a fixed value cannot
# be used to line the queue up
def _score_for(now_ms: int, tiebreak: int) -> int:
    mixed = (tiebreak ^ secrets.randbits(_TIEBREAK_BITS)) & _TIEBREAK_MASK
    elapsed_ms = max(now_ms - _QUEUE_EPOCH_MS, 0)
    return (elapsed_ms << _TIEBREAK_BITS) | mixed
