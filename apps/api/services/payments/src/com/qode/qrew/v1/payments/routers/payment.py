...
from com.qode.qrew.v1.payments.schemas.payment import (
...
# reports the payment status of a reservation, reconciling with stripe when it is still open
@router.get(
    "/{reservation_id}/payment",
    response_model=PaymentStatusResponse,
    summary="Read the payment status of a reservation",
)
@limiter.limit("120/minute")  # type: ignore[misc]
async def read_reservation_payment(
    request: Request,
    reservation_id: uuid.UUID,
    current_user: AuthenticatedUser = Depends(get_current_user),
    service: PaymentService = Depends(get_payment_service),
    ...
    del request
    payment = await service.get_for_reservation(reservation_id)
    if payment is None or payment.user_id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail={"message": "Payment not found.", "field": "reservation_id"},
        )
    payment = await service.sync_from_provider(payment)
