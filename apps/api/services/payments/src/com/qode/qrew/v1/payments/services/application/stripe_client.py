...
class StripeClient(Protocol):
    ...
    # reads the current state of a payment intent
    async def retrieve_payment_intent(self, intent_id: str) -> RetrievedIntent: ...
...
class StripeRealClient:
    ...
    # reads the current state of a payment intent from the stripe api
    async def retrieve_payment_intent(self, intent_id: str) -> RetrievedIntent:
        intent: Any = await anyio.to_thread.run_sync(  # type: ignore[misc]
            lambda: stripe.PaymentIntent.retrieve(intent_id)
        )
        raw_error: Any = getattr(intent, "last_payment_error", None)  # type: ignore[reportUnknownArgumentType]
        last_error: dict[str, Any] = (
            cast("dict[str, Any]", raw_error) if isinstance(raw_error, dict) else {}
        )
        code: Any = last_error.get("code")
        message: Any = last_error.get("message")
