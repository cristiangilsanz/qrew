...
from com.qode.qrew.v1.payments.services.application.audit import AuditService
...
class PaymentService:
    # stores the session repository stripe client and audit trail the service uses
    def __init__(
        self,
        session: AsyncSession,
    ...
    # writes one outcome of a payment onto the trail, where a failure to record must
    # never undo a charge the provider has already settled
    async def _record(self, action: str, payment: Payment) -> None:
        try:
            ...
            await logger.awarning("payment_audit_failed", action=action, error=str(exc))
    ...
    async def get_for_reservation(self, reservation_id: uuid.UUID) -> Payment | None:
        return await self._repo.get_by_reservation_id(reservation_id)

    # reads the outcome from stripe and applies it when the webhook has not arrived yet
    @traced("payment.sync_from_provider")
    async def sync_from_provider(self, payment: Payment) -> Payment:
        if is_terminal(payment.status) or not payment.provider_payment_intent_id:
            return payment
        intent_id = payment.provider_payment_intent_id
        try:
            intent = await self._stripe.retrieve_payment_intent(intent_id)
        except Exception:
            await logger.awarning("payment_sync_failed", intent_id=intent_id)
            return payment

        mapped = map_intent_status(intent.status)
        if mapped == payment.status:
            return payment
        if mapped is PaymentStatus.succeeded:
            await self.apply_succeeded(intent_id=intent_id)
        elif mapped is PaymentStatus.failed:
            await self.apply_failed(
                intent_id=intent_id,
                failure_code=intent.failure_code,
                failure_message=intent.failure_message,
            )
        else:
            await self.update_intermediate(intent_id=intent_id, status=intent.status)
        return payment
    ...
    async def apply_succeeded(self, *, intent_id: str) -> None:
        ...
        await self._record("PAYMENT_SUCCEEDED", payment)
    ...
    async def apply_failed(
        ...
        *,
        ...
        await self._record("PAYMENT_FAILED", payment)
    ...
    async def apply_refund(
        ...
        await self._record("PAYMENT_REFUNDED", payment)
