...
class DeviceBindingService:
    ...
    async def complete(
        ...
        # an account holds one device at a time, so losing the bound one is resolved by
        # revoking it rather than by accumulating bindings
        if existing is None:
            active = await self._device_repo.get_active_by_user_id(user.id)
            if active:
                raise DeviceBindingError(
