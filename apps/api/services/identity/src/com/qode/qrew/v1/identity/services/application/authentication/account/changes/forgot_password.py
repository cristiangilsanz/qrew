...
class ForgotPasswordService:
    ...
    async def reset_password(self, token: str, new_password: str) -> None:
        ...
        # the lookup already matched on the hash, so this only re-checks it in
        # constant time in case a row came back without one
        if (
            user is None
            or user.password_reset_token is None
            or not compare_digest(user.password_reset_token, pii_crypto.hash_lookup(token))
        ):
