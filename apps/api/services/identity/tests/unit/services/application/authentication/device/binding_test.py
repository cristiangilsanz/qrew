...
def _service(existing: MagicMock | None) -> tuple[DeviceBindingService, MagicMock]:
    ...
    # the one device per account cap reads the active ones before creating another
    repo.get_active_by_user_id = AsyncMock(return_value=[])
...
class TestBindingAKeyAlreadyOnRecord:
    ...
    # verifies that an account already holding a device refuses a second one, so the
    # lost handset is resolved by revoking it rather than by accumulating bindings
    @pytest.mark.asyncio
    async def test_it_refuses_a_second_device_for_the_same_account(self) -> None:
        user = make_user()
        service, repo = _service(None)
        repo.get_active_by_user_id = AsyncMock(return_value=[_device(user.id, revoked=False)])
        with pytest.raises(DeviceBindingError, match="already bound"):
            await _complete(service, user)
        repo.create.assert_not_awaited()

    # verifies that reviving one's own revoked device is not treated as a second one
    @pytest.mark.asyncio
    async def test_it_still_revives_a_revoked_device_despite_the_cap(self) -> None:
        user = make_user()
        existing = _device(user.id, revoked=True)
        service, repo = _service(existing)
        repo.get_active_by_user_id = AsyncMock(return_value=[_device(user.id, revoked=False)])
