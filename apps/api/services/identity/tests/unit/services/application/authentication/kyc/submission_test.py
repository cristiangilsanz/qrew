...
class TestNationalIdHash:
    # verifies that the document number is hashed with the shared domain separated
    # helper, so a number cannot be recovered from a table of bare sha256 digests
    async def test_the_document_number_is_not_hashed_bare(self) -> None:
        service, _, _ = _make_service(scanned="00000001R")
        user = _user()
        with patch(f"{_MODULE}.settings", _settings()):
            await _upload(service, user, document_type="dni", document_number="00000001R")
        assert user.national_id_hash != hashlib.sha256(b"00000001R").hexdigest()
        assert user.national_id_hash == pii_crypto.hash_lookup("00000001R")

    # verifies that the duplicate check looks the same hash up that it stores
    async def test_the_duplicate_check_uses_the_same_hash(self) -> None:
        service, repo, _ = _make_service(scanned="00000001R")
        with patch(f"{_MODULE}.settings", _settings()):
            await _upload(service, _user(), document_type="dni", document_number="00000001R")
        repo.get_by_national_id_hash.assert_awaited_once_with(pii_crypto.hash_lookup("00000001R"))
