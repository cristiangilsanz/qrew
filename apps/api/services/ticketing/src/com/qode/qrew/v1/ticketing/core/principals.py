...
# reads the presence claims a vouched request still carries in its own token, and
# reports neither when there is no token or it does not verify
def _presence_from_token(
    ...
    return device_id, last_asserted_at
...
def get_current_user(
    ...
    if user_id_str:
        ...
        # the gateway names the caller, but the moment of the last passkey assertion and
        # the device it came from ride in the token itself, and the gate needs both
        device_id, last_asserted_at = _presence_from_token(credentials)
