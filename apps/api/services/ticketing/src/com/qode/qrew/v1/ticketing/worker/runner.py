...
import com.qode.qrew.v1.ticketing.worker.jobs.outbox_drainer  # noqa: F401  # pyright: ignore[reportUnusedImport]
...
async def _on_startup(ctx: dict[str, Any]) -> None:
    del ctx
    install_drain_notifier("ticketing", redis_url=settings.redis_url)
    ...
    if not settings.nats_url:
        await logger.awarning("ticketing_arq_worker.no_nats_url")
        return
