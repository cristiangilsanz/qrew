    ...
    *,
    ...
    if settings.ticket_qr_require_attestation:
        ...
        if now - attested > timedelta(hours=settings.ticket_qr_attestation_max_age_hours):
            return DenialReason.attestation
    # the handset itself flagged the fix as simulated; only attestation makes the
    # opposite claim worth anything, and an absent signal is a browser or an older
    # handset rather than an accusation
    if settings.ticket_qr_require_location_integrity and location_is_mock is True:
        return DenialReason.location_mock
