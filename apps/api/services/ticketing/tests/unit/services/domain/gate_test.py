...
def _settings(
    *,
    ...
    return SimpleNamespace(
        ...
        ticket_qr_require_location_integrity=location_integrity,
...
class TestLocationIntegrity:
    # verifies that a fix the handset itself calls simulated is refused
    def test_denies_a_simulated_fix(self) -> None:
        with patch(
            _PATCH_SETTINGS,
            _settings(attestation=False, device_binding=False, location_integrity=True),
        ):
            reason = evaluate_gate(
                _inputs(),
                last_asserted_at=NOW,
                latitude=40.0,
                longitude=-3.0,
                location_is_mock=True,
                now=NOW,
            )
        assert reason is DenialReason.location_mock

    # verifies that a fix the handset vouches for is let through
    def test_allows_a_genuine_fix(self) -> None:
        with patch(
            _PATCH_SETTINGS,
            _settings(attestation=False, device_binding=False, location_integrity=True),
        ):
            reason = evaluate_gate(
                ...
                latitude=40.0,
                longitude=-3.0,
                location_is_mock=False,
                now=NOW,
            )
        assert reason is None

    # verifies that a platform with no such signal is not accused of faking one
    def test_allows_a_caller_that_reports_no_signal(self) -> None:
        with patch(
            _PATCH_SETTINGS,
            _settings(attestation=False, device_binding=False, location_integrity=True),
        ):
            reason = evaluate_gate(
                ...
                latitude=40.0,
                longitude=-3.0,
                location_is_mock=None,
                now=NOW,
            )
        assert reason is None

    # verifies that the caller may omit the signal entirely
    def test_allows_a_caller_that_omits_the_signal(self) -> None:
        with patch(
            _PATCH_SETTINGS,
            _settings(attestation=False, device_binding=False, location_integrity=True),
        ):
    ...
    # verifies that the switch turns the check off like every other gate rule
    def test_ignores_a_simulated_fix_when_the_check_is_off(self) -> None:
        with patch(
            _PATCH_SETTINGS,
            _settings(attestation=False, device_binding=False, location_integrity=False),
        ):
            reason = evaluate_gate(
                _inputs(),
                last_asserted_at=NOW,
    ...
    # verifies that a simulated fix is named for what it is and not as a fence miss
    def test_names_the_simulated_fix_even_when_it_sits_outside_the_fence(self) -> None:
        with patch(
            _PATCH_SETTINGS,
            _settings(attestation=False, device_binding=False, location_integrity=True),
        ):
    ...
    # verifies that the check answers on its own and not through the fence switch
    def test_denies_a_simulated_fix_even_when_the_fence_is_off(self) -> None:
        with patch(
            _PATCH_SETTINGS,
            _settings(
                attestation=False,
