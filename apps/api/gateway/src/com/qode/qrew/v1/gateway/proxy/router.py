...
def _build_upstream_headers(request: Request) -> dict[str, str]:
    # a client may not name itself to an upstream service: every trusted header is
    # dropped here whatever it says, and only what the auth middleware vouched for
    # is put back, so a forged one cannot survive even if the middleware is skipped
    headers: dict[str, str] = {
        k: v
        for k, v in request.headers.items()
        if k.lower() not in _HOP_BY_HOP and not is_spoofable_header(k)
    }
    headers.update(request.scope.get(IDENTITY_SCOPE_KEY) or {})
