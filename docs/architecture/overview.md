...
transaction as the change that caused it, and a drainer job hands it to the broker
afterwards. Committing that transaction also enqueues the drain job, so the drainer runs
within the same second rather than on its next minute tick, and the every-minute cron stays
as the safety net for whenever that notification cannot be enqueued. The request therefore
commits or fails as a whole, and an unreachable broker delays the event rather than losing
it. Two things still leave the request path
