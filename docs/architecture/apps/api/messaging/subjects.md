...
The stream name is lowercase, unlike every other one, because the entry subscriber that creates it spells it that way.

| Subject | Published by | Consumed by | Description |
|---|---|---|---|
| `ticketing.ticket.state_changed.v1` | Ticketing | Entry | Ticket state transition occurred |
| `ticketing.ticket.restored.v1` | Ticketing | | Frozen ticket rebound to a device the holder re-enrolled |
...
| Subject | Published by | Consumed by | Description |
|---|---|---|---|
| `audit.events.v1` | Identity, Catalog, Sales, Payments, Ticketing, Entry | Audit | Security and business audit record. Only the Gateway writes none |
