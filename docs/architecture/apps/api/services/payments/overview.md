...
|--------|------|-------------|
| `outbox_drainer` | arq job, every minute and on notification | Publishes to NATS every domain event waiting in `payments.event_outbox`, retrying with backoff and parking a row after eight attempts. Enqueued right after a transaction records an event, with the cron as the safety net. |
