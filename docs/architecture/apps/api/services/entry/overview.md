...
|-------|-------------|--------|
| `TicketStateChanged` | `ticketing.ticket.state_changed.v1` | Updates the local `TicketContext` projection used for scan validation. |
...
|--------|------|-------------|
| `ticketing.ticket.state_changed.v1` | NATS subscriber | Keeps the local ticket state projection up to date. |
