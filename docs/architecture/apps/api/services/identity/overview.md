...
| `event_outbox_drainer` | arq job, every minute and on notification | Publishes to NATS every domain event waiting in `identity.event_outbox`, retrying with backoff and parking a row after eight attempts. Enqueued right after a transaction records an event, with the cron as the safety net. |
...
| `outbox` | Transactional outbox mixin, recorder, drainer, and post-commit drain notifier for domain events |
