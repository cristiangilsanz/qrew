...
async def write(
    ...
    for venue in data.venues:
        await conn.execute(
            ...
            venue.radius_m,
            venue.timezone,
            venue.description or f"{venue.name} in {venue.city}.",
            when.days(-58),
        )
    ...
    for event in data.events:
        ...
        announced = event.sale_starts_at(when)
        created = min(when.days(-50), announced - timedelta(days=10))
        published = (
            None
            if event.status == "draft"
            else min(when.days(-40), announced - timedelta(days=1))
        )
        await conn.execute(
            """
        ...
        for tier in event.ticket_types:
            await conn.execute(
                ...
                event.id,
                tier.name,
                tier.description or f"{tier.name} access.",
                tier.capacity,
                tier.reserved,
                tier.price_cents,
                tier.position,
