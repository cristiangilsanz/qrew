...
from com.qode.qrew.v1.service.routers.auth import get_passkey_reassertion_service
from com.qode.qrew.v1.service.services.passkey import (
    PasskeyError,
    PasskeyReassertionService,
)
from com.qode.qrew.v1.service.services.passkey import reassertion as _reassertion_module
...
def mock_service() -> AsyncMock:
    ...
    service = AsyncMock()
    service.begin = AsyncMock(return_value=_FAKE_OPTIONS)
    service.complete = AsyncMock(return_value=asserted_at)
    return service


@pytest.fixture(autouse=True)
def override_dependencies(mock_service: AsyncMock) -> Iterator[None]:
    app.dependency_overrides[get_passkey_reassertion_service] = lambda: mock_service
...
async def test_begin_passes_session_jti_to_service(
    ...
    args = mock_service.begin.call_args.args
...
async def test_begin_returns_400_when_no_passkey_registered(
    ...
    mock_service.begin.side_effect = PasskeyError(
...
async def test_complete_returns_400_when_challenge_expired(
    ...
    mock_service.complete.side_effect = PasskeyError(
...
async def test_service_stamps_last_asserted_at(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        _reassertion_module.webauthn, "verify_authentication_response", _fake_verify
    )
    monkeypatch.setattr(_reassertion_module, "base64url_to_bytes", _fake_decode)

    svc = PasskeyReassertionService(passkey_repo, redis, audit, session_repo)
    request = MagicMock()
    ...
    result = await svc.complete(user, session, request)
...
async def test_service_raises_when_credential_belongs_to_other_user() -> None:
    ...
    credential = MagicMock()
    credential.user_id = uuid.uuid4()

    passkey_repo = AsyncMock()
    passkey_repo.get_by_credential_id = AsyncMock(return_value=credential)

    svc = PasskeyReassertionService(passkey_repo, AsyncMock(), AsyncMock(), AsyncMock())
    request = MagicMock()
    request.raw_id = "Y2hlY2tNZQ"

    with pytest.raises(PasskeyError, match="not recognised"):
        await svc.complete(user, session, request)
...
async def test_service_raises_when_challenge_missing() -> None:
    ...
    svc = PasskeyReassertionService(passkey_repo, redis, AsyncMock(), AsyncMock())
    request = MagicMock()
    request.raw_id = "Y2hlY2tNZQ"
...
async def test_begin_reassertion_stores_30s_challenge(
    ...
    monkeypatch.setattr(
        _reassertion_module.webauthn, "generate_authentication_options", _fake_gen
    )
    monkeypatch.setattr(_reassertion_module.webauthn, "options_to_json", _fake_to_json)

    svc = PasskeyReassertionService(passkey_repo, redis, AsyncMock(), AsyncMock())
    result = await svc.begin(user, session_jti)
...
async def test_begin_reassertion_raises_when_no_passkey() -> None:
    ...
    with pytest.raises(PasskeyError, match="No passkey"):
        await svc.begin(user, str(uuid.uuid4()))
