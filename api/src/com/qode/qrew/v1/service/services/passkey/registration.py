...
from com.qode.qrew.v1.service.services.passkey._common import (
...
class PasskeyRegistrationService:
    ...
    async def begin(self, user: User) -> str:
        ...
        await self._redis.set(
            challenge_key(user.id), options.challenge, ex=CHALLENGE_TTL_SECONDS
        )
        await logger.ainfo("passkey_registration_begin", user_id=str(user.id))
        return webauthn.options_to_json(options)

    async def complete(
        self, user: User, request: PasskeyRegistrationCompleteRequest
    ) -> None:
